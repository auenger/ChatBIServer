...
export default function WorkbenchPanel({ initialDatasourceId, initialSql }: WorkbenchPanelProps = {}) {
  const { styles } = useStyles();
  const [profiles, setProfiles] = useState<DataSourceProfile[]>([]);
  const [datasourceId, setDatasourceId] = useState<number | null>(initialDatasourceId ?? null);
  ...
  useEffect(() => {
    datasourceApi.list().then((list) => {
      setProfiles(list);
      const initial = initialDatasourceId ?? list[0]?.id;
